...
// Prepare SQL for products and item count based on search query and type
$typeId = 2; // TypeId 2 for dental products
$searchQueryParam = "%$searchQuery%";

// SQL query for products with search filter
$sql = "SELECT ProductID, product_image, Name, Price, description 
        FROM tbl_product 
        WHERE typeid = :typeId AND Name LIKE :searchQuery 
        LIMIT :start, :perPage";

// SQL query to count items matching the search query
$countSql = "SELECT COUNT(*) AS item_count 
             FROM tbl_product 
             WHERE typeid = :typeId AND Name LIKE :searchQuery";
...
try {
    ...
    $stmt->bindParam(':searchQuery', $searchQueryParam, PDO::PARAM_STR);
    ...
    $countStmt->bindParam(':typeId', $typeId, PDO::PARAM_INT);
    $countStmt->bindParam(':searchQuery', $searchQueryParam, PDO::PARAM_STR);
    $countStmt->execute();
    $item_count = $countStmt->fetch(PDO::FETCH_ASSOC)['item_count'];
    $totalPages = ceil($item_count / $perPage);

} catch (PDOException $e) {
    die("Query failed: " . $e->getMessage());
}

// Clear the search query from the session after use
  ...
  <div class="col-md-12">
      <div class="row justify-content-center mb-3" id="productList">
        <div class="col-md-12">
          ...
          <div class="card cards shadow-0 border rounded-3 mb-3">
            <div class="card-body">
              <div class="row g-0">
